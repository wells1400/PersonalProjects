...
def build_LSTM_Model(inputDim = 10,lr=0.001,nb_hidden_cell=[80]):
    ...
    model.add(LSTM(nb_hidden_cell[0],input_shape=(1,inputDim),use_bias=True,activation='relu',name='layer_0',return_sequences=True))
...
def model_fit(model,x_train,y_trian,validation_split=0.3,saveFile='D:\WORK__wells\PROGRAM_3\Model pic',epochs=10,batch_size=30):
    from keras.callbacks import ModelCheckpoint
    #创建一个实例LossHistory
    history = LossHistory()
#     early_stopping = EarlyStopping(monitor='val_loss', patience=50)
    checkpointer = ModelCheckpoint(filepath="D:\WORK__wells\PROGRAM_3\Model pic\checkpoint.hdf5",
    ...
    model.save(r'D:\WORK__wells\PROGRAM_3\Model pic\Model.h5')
...
def model_predict(model,x_test,y_test,scaler,title='predict',save=r'D:\WORK__wells\PROGRAM_3\Model pic\predict.png',Y_reverse=1):
...
#     saveFig = r'D:\WORK__wells\PROGRAM_3\Model pic\Predict.png'
    plt.savefig(save, dpi=2400)
    plt.show()
    print('RMSE:%s  ,MAE:%s  ,MAPE:%s '%(RMSE,MAE,MAPE))
    return RMSE,MAE,MAPE,predict_array,y_test

def model_predictVal(model,x_test,scaler,title='predict',save=r'D:\WORK__wells\PROGRAM_3\Model pic\predict.png',**kwargs):
    predict_array = model.predict(x_test)
#     predict_array = scaler.inverse_transform(predict_array.reshape(predict_array.shape[0],1))
...
class LossHistory(cb):
    ...
    def on_batch_end(self, batch, logs={}):
        ...
        self.val_mae['batch'].append(logs.get('maval_maee'))
